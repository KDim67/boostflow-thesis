...
/**
 * Helper to get the correct icon based on milestone status
 */
...
/**
 * Helper to get the correct CSS classes based on milestone status
 */
...
/**
 * Given a current milestone status, returns the next state in the sequence
 */
const getNextMilestoneStatus = (status: string): MilestoneStatus => {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white dark:bg-gray-800 rounded-lg p-6 w-full max-w-md mx-4">
        ...
        <div className="space-y-4">
          <div>
            <label
              htmlFor="project-name-44ob8"
              className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
            >
              Project Name
            </label>{" "}
            <input
              id="project-name-44ob8"
              type="text"
              value={editForm.name}
              onChange={(e) =>
          ...
          <div>
            <label
              htmlFor="progress-editform-progress-06fdp"
              className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1"
            >
              Progress ({editForm.progress}%)
            </label>{" "}
            <input
              id="progress-editform-progress-06fdp"
              type="range"
              min="0"
              max="100"
                  ...
                  <li key={milestone.id} className="py-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center flex-1">
                        <button
                          type="button"
                          tabIndex={canAccessAdvancedFeatures ? 0 : -1}
                          className={`h-8 w-8 rounded-full flex items-center justify-center mr-3 border-none ${canAccessAdvancedFeatures ? "cursor-pointer" : "cursor-default"} ${getMilestoneStatusColor(milestone.status)}`}
                          onClick={() => {
                            if (canAccessAdvancedFeatures) {
                              const nextStatus = getNextMilestoneStatus(
                                milestone.status
                              );
                              handleMilestoneStatusChange(
                                milestone.id,
              ...
              <>
                ...
                {project.teamMembers.length > 5 && (
                  <div className="mt-4 text-center">
                    ...
                    >
                      <span>
                        View All Team Members ({project.teamMembers.length})
                      </span>
                      <svg
        ...
        <div className="p-6">
          ...
          {activeTab === "overview" && (
            <ProjectOverviewTab
              ...
              canAccessAdvancedFeatures={canAccessAdvancedFeatures}
              handleAddMilestone={handleAddMilestone}
              handleMilestoneStatusChange={handleMilestoneStatusChange}
