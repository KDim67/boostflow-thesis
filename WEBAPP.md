_This document covers the BoostFlow web application. For the DevSecOps pipeline documentation (the thesis subject), see [README.md](README.md)._

# BoostFlow - Web Application
...
1. **Clone the repository:**
   ...
   ```bash
   git clone https://github.com/KDim67/boostflow-thesis.git
   cd boostflow-thesis
   ```
...
The `docker-compose.yaml` provides a containerized local development environment with two services:

- **app**: Builds the application from the Dockerfile, exposes port `3000`, and depends on MinIO being healthy
- **minio**: MinIO object storage for file uploads, exposes API on port `9000` and console on port `9001`

Both services read configuration from `.env.local`.
